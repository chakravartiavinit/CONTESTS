...
int main()
{
    ...
    while (t--)
    {
        ...
        string alpha;
        cin >> alpha;
        vector<pair<int, int>> positions, positions1;
        for (int i = 0; i < alpha.length(); ++i)
        {
            if (alpha[i] == '1')
            {
                positions.push_back({vt[i], i});
            }
            else
            {
                positions1.push_back({vt[i], i});
            }
        }
        vector<int> answer(n);
        ...
        {
            answer[positions[index].second] = temp;
            index++;
            temp--;
        }
        index = 0;
        // debug(answer);
        while (index <positions1.size())
        {
            answer[positions1[index].second] = temp;
            index++;
            temp--;
        }
        for (auto x : answer)
